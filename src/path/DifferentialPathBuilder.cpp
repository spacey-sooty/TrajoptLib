...
void DifferentialPathBuilder::WptObstacle(size_t index, const Obstacle& obstacle) {
...
void DifferentialPathBuilder::ControlIntervalCounts(std::vector<size_t>&& counts) {
  controlIntervalCounts = std::move(counts);
}

const std::vector<size_t>& DifferentialPathBuilder::GetControlIntervalCounts() const {
  return controlIntervalCounts;
}

DifferentialSolution DifferentialPathBuilder::CalculateInitialGuess() const {
  return GenerateLinearInitialGuess<DifferentialSolution>(initialGuessPoints,
